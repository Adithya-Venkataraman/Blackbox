...
def normalize(s):
    """Strip punctuation/whitespace for basic cleaning."""
    s = s.lower().strip()
...
def check_answer(chal_id, submitted):
    """
    Return True if submitted answer matches ANY keyword group for this challenge.
    A group matches when every keyword in that group appears in the submitted string.
    """
    cleaned = normalize(submitted)
    groups  = ANSWER_KEYWORDS.get(int(chal_id), [])
    for group in groups:
